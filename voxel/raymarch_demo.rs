...
use helio::{
    required_experimental_features, required_wgpu_features, required_wgpu_limits, BrickRange,
    Camera, GpuLight, LightType, RenderGraph, RenderPass, Renderer, RendererConfig, Scene,
    SceneActor, VoxelMode, VoxelTerrain, VoxelVolumeDescriptor, VoxelVolumeId,
...
impl AppState {
    ...
    fn place_edit(
        ...
        world.paint_sphere(center_grid, radius_grid, material, add)
    }
}
...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        // Procedurally generate the CPU asset and hand its raymarch bricks to
        // SceneDB's stable residency for this exact volume.
        let world_seed = 1;
        let mut world = VoxelTerrain::empty();
        world.generate(world_seed);
        scene
            .upload_voxel_terrain(vol_id, &world)
            .expect("Failed to upload voxel terrain residency");
                ...
                match code {
                    ...
                    KeyCode::KeyR => {
                        ...
                        state
                            ...
                            .expect("Failed to regenerate voxel terrain residency");
                    }
                    _ => {}
                }
            }
            ...
            WindowEvent::MouseInput {
                    ...
                    AppState::place_edit(true, mat, pos, yaw, pitch, &mut state.world)
                {
                    state
                        .renderer
                        .scene_mut()
                        .upload_voxel_terrain_range(state.vol_id, &state.world, range)
                        .expect("Failed to upload edited voxel terrain range");
                }
            }
