...
use trueos_helio_runtime::{Camera, Error, linear_rgba_to_srgba8};
...
fn portal_clip_planes(portal: Portal, half: f32, projection: Projection) -> Option<Vec<ClipPlane>> {
    ...
    for (index, point) in local.iter().enumerate() {
        clip[index] = projection.project(add(
            center,
            add(scale(right, point[0]), scale(up, point[1])),
        ));
...
fn portal_point(portal: Portal, half: f32, local: [f32; 3]) -> [f32; 3] {
    ...
    add(
        ...
        add(
            scale(right, local[0]),
            add(scale(up, local[1]), scale(portal.normal, -local[2])),
        ),
    )
}
