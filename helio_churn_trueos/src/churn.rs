...
use trueos_helio_runtime::retained_transform::{
    RetainedTransformProgram, RetainedTransformTemplate, TransformHierarchyFrame,
};
use trueos_helio_runtime::{
    Camera, DrawIndexedIndirectArgs, Error, Projector, linear_rgba_to_srgba8,
};
...
fn decode_lighting(
    ...
    (
        [f32; 3],
        f32,
        [PointLight; LIGHT_COUNT],
        [[f32; 2]; MATERIAL_COUNT],
    ),
    Error,
> {
...
mod tests {
    ...
    fn helio_gpu_abi_lengths_offsets_and_little_endian_encoding_are_exact() {
        ...
        assert_eq!(
            GpuRetainedTransformSeed::pack_slot_and_flags(0x1_0000, 0),
            None
        );
        assert_eq!(
            GpuRetainedTransformSeed::pack_slot_and_flags(0, 0x1_0000),
            None
        );
    ...
    fn transform_frame_is_compact_prefix_counted_and_temporal() {
        ...
        let (first_translations, first_view_proj) = {
            ...
            assert_eq!(
                expected_slots,
                frame.draw_templates.map(|template| template.capacity)
            );
