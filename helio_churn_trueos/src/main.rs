...
const FRAME_X: i32 = 144;
const FRAME_Y: i32 = 96;
const INITIAL_WIDTH: u32 = 960;
const INITIAL_HEIGHT: u32 = 600;
const FRAME_MS: u64 = 16;
const CLEAR: u32 = u32::from_le_bytes([7, 9, 15, 255]);
...
fn main() {
    if let Err(stage) = run() {
        logl::log(
            logl::level::ERROR,
            format_args!("helio_churn_trueos: stopped at {stage}"),
        );
    }
}

fn run() -> Result<(), &'static str> {
    let spec = churn::Spec::decode_artifact(SCENE_ARTIFACT).map_err(|_| "scene decode")?;
    let mut engine = churn::Engine::new(spec).map_err(|_| "scene create")?;
    ...
    let mut next_status_ms = previous_ms;

    logl::log(
        logl::level::INFO,
        "helio_churn_trueos: ready; WASD/Space/Shift move, primary-drag looks, C toggles collisions, +/- changes churn",
    );
    ...
    loop {
        ...
        if now_ms >= next_status_ms {
            logl::log(
                logl::level::INFO,
                format_args!(
                    "helio_churn_trueos: objects={} spawn_rate={} collisions={}",
                    engine.active_objects(),
                    engine.spawn_rate(),
                    engine.collisions_enabled(),
                ),
            );
            next_status_ms = now_ms.saturating_add(5_000);
        }
...
fn drain_actions(frame: &mut Frame, engine: &mut churn::Engine) -> Result<(), &'static str> {
    while let Some(event) = frame.take_keyboard_event().map_err(|_| "keyboard event")? {
        ...
        match char::from_u32(event.codepoint) {
            Some('c' | 'C') => {
                let enabled = engine.toggle_collisions();
                logl::log(
                    logl::level::INFO,
                    format_args!("helio_churn_trueos: collisions={enabled}"),
                );
            }
            Some('+' | '=') => engine.adjust_spawn_rate(1),
            Some('-' | '_') => engine.adjust_spawn_rate(-1),
            _ => {}
        }
    }
    Ok(())
...
impl Renderer {
    fn new(batches: &[churn::Batch]) -> Result<Self, i32> {
