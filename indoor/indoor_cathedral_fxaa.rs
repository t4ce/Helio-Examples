//! Indoor cathedral example with FXAA (full-resolution, no temporal AA)
//!
...
//!
//! This demo uses the FXAA pipeline which renders at full native resolution
//! and applies a single-pass FXAA post-process instead of temporal
...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
            ...
            event_loop
                .create_window(
                    Window::default_attributes()
                        .with_title("Helio – Indoor Cathedral (FXAA)")
        ...
        // Switch to FXAA pipeline: full-res geometry, no temporal jitter/upscaling
        let config = RendererConfig::new(size.width, size.height, format)
            .with_shadow_quality(helio::ShadowQuality::Ultra)
            .with_render_scale(1.0);
