...
use helio::{
    ...
    RendererConfig, Scene, TextureSamplerDesc, TextureUpload,
};
...
fn make_decal(
    ...
    GpuDecal {
        transform: decal_xform(pos, normal, half),
        color,
        ...
        decal_type: dtype as u32,
        fade_time: 0.0,
        fade_start_delay: 0.0,
        age: 0.0,
        normal_adapt: if adapt { 1 } else { 0 },
        _pad0: 0.0,
        _pad1: 0.0,
    }
}
...
fn ring_pixels() -> Vec<u8> {
    ...
    for y in 0..RING_SIZE {
        for x in 0..RING_SIZE {
            ...
            let alpha =
                ((r - 0.45) / 0.1).clamp(0.0, 1.0) * (1.0 - ((r - 0.85) / 0.1).clamp(0.0, 1.0));
...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        for z in (-24..24).step_by(4) {
            let _ = renderer
                .scene_mut()
                .insert_actor(helio::SceneActor::decal(make_decal_tex(
                    [0.0, 0.0, z as f32],
                    [0.0, 1.0, 0.0],
                    [1.8, 1.2, 0.02],
                    [1.0, 1.0, 1.0, 0.9],
